...
import { StoredDocument } from './firebaseService';

// Helper to clean base64 string
const cleanBase64 = (b64: string) =>
  b64.replace(/^data:[a-zA-Z0-9-+\/]+;base64,/, '');
...
export const extractDocumentData = async (
  ...
  if (!apiKey) {
    console.error(
      'API Key missing. Checked: GEMINI_API_KEY, NEXT_PUBLIC_GENAI_API_KEY, API_KEY'
    );
  ...
  const prompt = `
    ...
    0. **Date Check**: TODAY'S DATE is ${new Date().toLocaleDateString(
      'en-US',
      {
        year: 'numeric',
        month: 'long',
...
export const verifyIdentityAndLiveness = async (
  ...
  if (storedDoc) {
    parts.push({
      inlineData: {
        ...
        data: cleanBase64(storedDoc.data),
      },
    }); // Image 3: Stored Doc
    prompt += `
  ...
  try {
    ...
    return {
      ...
      confidence: 0,
      reason:
        lang === 'es' ? 'Error de conexión con IA.' : 'AI Connection Error.',
    };
  }
};
